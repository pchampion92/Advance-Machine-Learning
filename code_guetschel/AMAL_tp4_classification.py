...
from AMAL_tp4_datasets import DataHolder, DatasetTempClassif
...
if __name__=="__main__":
    ...
    def get_dataloaders(sample_length, strides):
        train_dataset, val_dataset, test_dataset = data_holder.get_datasets_classif(sample_length, strides=strides)
    ...
    ## network definition :
    inDim = 1 + 3 # température * month * day * hour
    outDim = data_holder.get_num_classes()
    hidenDim = 10#2*outDim
    fc_layers = [] #[2*outDim]
    net = SequenceClassifier(inDim=inDim, hidenDim=hidenDim, outDim=outDim, fc_layers=fc_layers)
